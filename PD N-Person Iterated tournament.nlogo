...
to store-initial-turtle-counts
  ...
  set num-grofman n-grofman
  set num-shubik n-shubik
  set num-my-strat n-my-strat
end
...
to make-turtles
  ...
  create-turtles num-grofman [ set strategy "grofman" set color turquoise - 1 ]
  create-turtles num-shubik [set strategy "shubik" set color magenta ]
  create-turtles num-my-strat [set strategy "my-strat" set color yellow ]
...
to select-action ;;turtle procedure
  ...
  if strategy = "grofman" [ grofman ]
  if strategy = "shubik" [ shubik ]
  if strategy = "my-strat" [ my-strat ]
end
...
to play-a-round ;;turtle procedure
  ...
  if num-cooperate-games > 0 [
    set cooperate_av (cooperate-score / num-cooperate-games)
  ]
  if num-random-games > 0 [
    set random_av (random-score / num-random-games)
  ]
  if num-defect-games > 0 [
    set defect_av (defect-score / num-defect-games)
  ]
  if num-tit-for-tat-games > 0 [
    set tit-for-tat_av (tit-for-tat-score / num-tit-for-tat-games)
  ]
  if num-grofman-games > 0 [
    set grofman_av (grofman-score / num-grofman-games)
  ]
  if num-shubik-games > 0 [
    set shubik_av (shubik-score / num-shubik-games)
  ]
...
to update-history
  ...
  if strategy = "grofman" [ grofman-history-update ]
  if strategy = "shubik" [ shubik-history-update ]
  if strategy = "my-strat" [ my-strat-history-update ]
end
...
to grofman
  set num-grofman-games num-grofman-games + 1
  set partner-defected? item ([who] of partner) partner-history
  ifelse (partner-defected? = defect-now?) [
    if (random-float 0.7 < 0.2) [
    set defect-now? false
    ]
  ][set defect-now? true]
...
to grofman-history-update
  set partner-history
    (replace-item ([who] of partner) partner-history partner-defected?)
end

to shubik
  set num-shubik-games num-shubik-games + 1
  set partner-defected? item ([who] of partner) partner-history
...
to do-scoring
  ...
  set grofman-score  (calc-score "grofman" num-grofman)
  set shubik-score  (calc-score "shubik" num-shubik)
  set my-strat-score  (calc-score "my-strat" num-my-strat)
end
...
to setup-history
  set total-history table:make
  foreach [who] of turtles [
    x ->
    foreach [who] of turtles [
      y ->
      table:put total-history (list x y) []
    ]
  ]
end

to update-total-history
  foreach [who] of turtles with [partnered? = true] [
    x ->
    let y [who] of [partner] of turtle x
    let s1 "C"
    let s2 "C"
    if [defect-now?] of turtle x [set s1 "D"]
    if [defect-now?] of turtle y [set s2 "D"]
    let l table:get total-history (list x y)
    set l fput (list s1 s2) l
     table:put total-history (list x y) l
;    ]
    ]
...
"grofman" 1.0 0 -14835848 true "" "if num-grofman-games > 0 [ plot grofman-score / (num-grofman-games) ]"
"shubik" 1.0 0 -955883 true "" "if num-shubik-games > 0 [ plot shubik-score / (num-shubik-games) ]"
"my-strat" 1.0 0 -6459832 true "" "if num-my-strat-games > 0 [ plot my-strat-score / (num-my-strat-games) ]"
...
@#$#@#$#@
<experiments>
  <experiment name="experiment100" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="9000"/>
    <metric>count turtles</metric>
    <metric>random_av</metric>
    <metric>cooperate_av</metric>
    <metric>defect_av</metric>
    <metric>tit-for-tat_av</metric>
    <metric>grofman_av</metric>
    <metric>shubik_av</metric>
    <enumeratedValueSet variable="n-grofman">
      <value value="100"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="n-defect">
      <value value="100"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="n-tit-for-tat">
      <value value="100"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="n-shubik">
      <value value="100"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="n-random">
      <value value="100"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="n-cooperate">
      <value value="100"/>
    </enumeratedValueSet>
  </experiment>
</experiments>
